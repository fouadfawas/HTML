document.addEventListener('DOMContentLoaded', () => {
    ...
    const uploadSaveInput = document.getElementById('upload-save-input');
    ...
    async function loadAudio(url) {
        ...
        try {
            ...
            // Return null or a placeholder buffer if loading fails
            return null; 
        }
    }

    // Load sounds on startup
    // Changed paths to relative
    ...
    let musicSource = null; // Keep track of the source node
    let musicBuffer;
    ...
    async function loadMusic(url) {
         ...
         try {
            ...
            console.log("Music loaded.");
             ...
             console.error(`Error loading music from ${url}:`, e);
             musicBuffer = null; // Ensure buffer is null on failure
         }
    }

    function playMusic() {
        // Check if audioContext is created and running, and musicBuffer is loaded, and music is not already playing
        if (musicBuffer && audioContext && audioContext.state === 'running' && !musicPlaying) {
            musicSource = audioContext.createBufferSource(); // Create a new source node
            ...
            console.log("Music started.");
             ...
             console.log("Music not ready, already playing, or audio context not running.");
             console.log("Music Buffer:", musicBuffer);
             console.log("Audio Context:", audioContext ? audioContext.state : 'null');
    ...
    // Auto-play music is handled by the resumeAudioContext called on the first interaction
    // document.body.addEventListener('click', resumeAudioContext, { once: true }); // Moved into loadAudio
    ...
    function loadGame(gameState) {
         if (gameState) {
            // Load from provided state (e.g., from file upload)
            if (typeof gameState.score === 'number') {
                score = gameState.score;
            }
            if (typeof gameState.upgrades === 'object') {
                Object.keys(upgrades).forEach(key => {
                    if (gameState.upgrades.hasOwnProperty(key) && typeof gameState.upgrades[key].level === 'number') {
                        upgrades[key].level = gameState.upgrades[key].level;
                    }
                });
            }
             saveLoadStatus.textContent = "Game loaded successfully!";
            ...
            if (savedScore !== null) {
                score = parseFloat(savedScore); // Use parseFloat for potential fractional passive income
            }
            ...
            Object.keys(upgrades).forEach(key => {
             ...
             saveLoadStatus.textContent = "Game loaded from browser storage.";
             setTimeout(() => saveLoadStatus.textContent = "", 3000);
        }
    ...
    function saveGame() {
        ...
        Object.keys(upgrades).forEach(key => {
         ...
         // Update status briefly if not saving to file
        if (!document.activeElement || (document.activeElement !== downloadSaveButton && document.activeElement !== uploadSaveButton && document.activeElement !== uploadSaveInput)) {
             saveLoadStatus.textContent = "Game saved to browser storage.";
    ...
    function saveGameToFile() {
        ...
        Object.keys(upgrades).forEach(key => {
            gameState.upgrades[key] = { level: upgrades[key].level };
        });
    ...
    downloadSaveButton.addEventListener('click', saveGameToFile);
